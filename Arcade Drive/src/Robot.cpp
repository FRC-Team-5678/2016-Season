...
private:
	...
	void TeleopPeriodic() {
		...
		if(!override){
			myRobot.ArcadeDrive((-stick.GetY()*((-(((-stick.GetThrottle()+1)/2))*0.8))),(-stick.GetX()*0.65));
	...
	void TestPeriodic() {
		...
		in1y = (stick.GetY()*((-(((-stick.GetThrottle()+1)/2))/2)));
		in1x = (stick.GetX()*((-(((-stick.GetThrottle()+1)/2))/4)));
		in2y = (-Xbox1.GetY()*0.5);
		in2x = (Xbox1.GetX()*0.25);
